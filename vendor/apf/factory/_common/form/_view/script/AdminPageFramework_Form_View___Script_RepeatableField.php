...
class WP_RadarChartAdminPageFramework_Form_View___Script_RepeatableField extends WP_RadarChartAdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateWP_RadarChartAdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.wp-radar-chart-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.wp-radar-chart-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
...
// @todo event.preventDefault();        
            ...
            $( this ).addWP_RadarChartAdminPageFrameworkRepeatableField();
...
// @todo event.preventDefault();        
            $( this ).removeWP_RadarChartAdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.wp-radar-chart-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.wp-radar-chart-field' )
            .filter( function() {                 
                return $( this ).closest( '.wp-radar-chart-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addWP_RadarChartAdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addWP_RadarChartAdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.wp-radar-chart-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.wp-radar-chart-fields' );
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.wp-radar-chart-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.wp-radar-chart-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.wp-radar-chart-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.wp-radar-chart-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.wp-radar-chart-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.wp-radar-chart-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
         ...
         */
        nodeNewField.updateWP_RadarChartAdminPageFrameworkRepeatableFields();
        ...
        $( nodeNewField ).find( '.wp-radar-chart-field' ).addBack().trigger( 
            'wp-radar-chart_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.wp-radar-chart-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.wp-radar-chart-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
        ...
        nodeFieldsContainer.children( '.wp-radar-chart-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.wp-radar-chart-field,.wp-radar-chart-fields,.wp-radar-chart-fieldset' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeWP_RadarChartAdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.wp-radar-chart-field' );
        var nodeFieldsContainer = $( this ).closest( '.wp-radar-chart-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );

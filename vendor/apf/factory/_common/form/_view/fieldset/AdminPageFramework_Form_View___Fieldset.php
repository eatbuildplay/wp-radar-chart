...
abstract class WP_RadarChartAdminPageFramework_Form_View___Fieldset_Base extends WP_RadarChartAdminPageFramework_Form_Utility {
    ...
    private function _getFormatted($aFieldset, $aFieldTypeDefinitions) {
        return $this->uniteArrays($aFieldset, $this->_getFieldTypeDefaultArguments($aFieldset['type'], $aFieldTypeDefinitions) + WP_RadarChartAdminPageFramework_Form_Model___Format_Fieldset::$aStructure);
    }
    ...
    private function _loadScripts($sStructureType = '') {
        if ('widget' === $sStructureType && !self::$_bIsLoadedJSScripts_Widget) {
            new WP_RadarChartAdminPageFramework_Form_View___Script_Widget;
        ...
        new WP_RadarChartAdminPageFramework_Form_View___Script_Utility;
        new WP_RadarChartAdminPageFramework_Form_View___Script_OptionStorage;
        new WP_RadarChartAdminPageFramework_Form_View___Script_AttributeUpdator;
        new WP_RadarChartAdminPageFramework_Form_View___Script_RepeatableField($this->oMsg);
        new WP_RadarChartAdminPageFramework_Form_View___Script_SortableField;
    }
...
jQuery( document ).ready( function() {
    var _oButtonPlaceHolders = jQuery( '#{$sFieldsContainerID} > .wp-radar-chart-field.without-child-fields .repeatable-field-buttons' );
    ...
    /* Otherwise, insert the button element at the beginning of the field tag */
         ...
         */
        if ( ! jQuery( '#{$sFieldsContainerID} .wp-radar-chart-repeatable-field-buttons' ).length ) { 
            jQuery( '#{$sFieldsContainerID} > .wp-radar-chart-field.without-nested-fields' ).prepend( $_sSmallButtons );
        }
        /**
         * Support for nested fields.
         * For nested fields, add the buttons to the fields tag.
         */
        jQuery( '#{$sFieldsContainerID} > .wp-radar-chart-field.with-nested-fields' ).prepend( $_sNestedFieldsButtons );
        
        /**
         * Support for inline mixed fields.
         */
        // jQuery( '#{$sFieldsContainerID} > .wp-radar-chart-field.with-mixed-fields' ).prepend( $_sNestedFieldsButtons );
        
    }     
    jQuery( '#{$sFieldsContainerID}' ).updateWP_RadarChartAdminPageFrameworkRepeatableFields( $_aJSArray ); // Update the fields     
});
JAVASCRIPTS;
    ...
    private function ___getRepeatableButtonHTML($sFieldsContainerID, array $aArguments, $iFieldCount, $bSmall = true) {
        $_oFormatter = new WP_RadarChartAdminPageFramework_Form_Model___Format_RepeatableField($aArguments, $this->oMsg);
    ...
    private function ___getContainerAttributes($aArguments) {
        $_aContainerAttributes = array('class' => $this->getClassAttribute('wp-radar-chart-repeatable-field-buttons', !empty($aArguments['disabled']) ? 'disabled' : ''),);
    ...
    protected function _getSortableFieldEnablerScript($sFieldsContainerID) {
        $_sScript = <<<JAVASCRIPTS
    jQuery( document ).ready( function() {
        jQuery( this ).enableWP_RadarChartAdminPageFrameworkSortableFields( '$sFieldsContainerID' );
    });
JAVASCRIPTS;
        return "<script type='text/javascript' class='wp-radar-chart-sortable-field-enabler-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    }
    class WP_RadarChartAdminPageFramework_Form_View___Fieldset extends WP_RadarChartAdminPageFramework_Form_View___Fieldset_Base {
        public function get() {
            $_aOutputs = array();
            $_oFieldError = new WP_RadarChartAdminPageFramework_Form_View___Fieldset___FieldError($this->aErrors, $this->aFieldset['_section_path_array'], $this->aFieldset['_field_path_array'], $this->aFieldset['error_message']);
            $_aOutputs[] = $_oFieldError->get();
            $_oFieldsFormatter = new WP_RadarChartAdminPageFramework_Form_Model___Format_Fields($this->aFieldset, $this->aOptions);
        ...
        private function _getEachFieldOutput($aField, $isIndex, array $aCallbacks, $bIsLastElement = false) {
            ...
            $_oSubFieldFormatter = new WP_RadarChartAdminPageFramework_Form_Model___Format_EachField($aField, $isIndex, $aCallbacks, $_aFieldTypeDefinition);
        ...
        private function _getFieldOutput($sContent, $aField, $bIsLastElement) {
            $_oFieldAttribute = new WP_RadarChartAdminPageFramework_Form_View___Attribute_Field($aField);
        ...
        private function _getFinalOutput($aFieldset, array $aFieldsOutput, $iFieldsCount) {
            $_oFieldsetAttributes = new WP_RadarChartAdminPageFramework_Form_View___Attribute_Fieldset($aFieldset);
        ...
        private function _getEmbeddedFieldTitle($aFieldset) {
            ...
            $_oFieldTitle = new WP_RadarChartAdminPageFramework_Form_View___FieldTitle($aFieldset, '', $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
        ...
        private function _getChildFieldTitle($aFieldset) {
            ...
            $_oFieldTitle = new WP_RadarChartAdminPageFramework_Form_View___FieldTitle($aFieldset, array('wp-radar-chart-child-field-title'), $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
            return $_oFieldTitle->get();
        }
        private function _getFieldsetContent($aFieldset, $aFieldsOutput, $iFieldsCount) {
            ...
            $_oFieldsAttributes = new WP_RadarChartAdminPageFramework_Form_View___Attribute_Fields($aFieldset, array(), $iFieldsCount);
        ...
        private function _getExtras($aField, $iFieldsCount) {
            $_aOutput = array();
            $_oFieldDescription = new WP_RadarChartAdminPageFramework_Form_View___Description($aField['description'], 'wp-radar-chart-fields-description');
